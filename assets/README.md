...
Encoder and index packages from different embedding spaces must never be mixed.
Reproducible commands and output locations are documented in the repository
README. Every generated bundle contains checksums and refuses to load unless
its manifest is finalized. The BGE license and source notice are in
`BGE_MODEL_ATTRIBUTION.md` at the repository root and inside the model bundle.
