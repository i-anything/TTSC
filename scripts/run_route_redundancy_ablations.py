"""Sealed aggregate-only Phase 12 generator-separated evaluation."""
...
from scripts.verify_phase12_route_redundancy_oracle import (
    EXPECTED_SHA256 as PHASE12_ORACLE_SHA256,
    RANDOM_CASES as PHASE12_RANDOM_ORACLE_CASES,
    verify as verify_phase12_oracle,
)
...
EXPERIMENT_ID = "phase12-route-redundancy-corrected-stage-a-v1"
CANDIDATE_ID = ROUTE_REDUNDANCY_CORRECTED_RANKING_POLICY.value
BASELINE_ID = "phase9-bounded-profile-residual-v1"
IMPLEMENTATION_LOCK_ID = "phase12-route-redundancy-implementation-v1"
IMPLEMENTATION_LOCK_RELATIVE = "docs/phase12_implementation_lock.json"
CONTRACT_RELATIVE = "docs/phase12_experiment_contract.json"
BASELINE_LOCK_RELATIVE = "docs/phase12_baseline_lock.json"
DATASET_AUDIT_RELATIVE = "docs/phase11_dataset_audit.json"
RESEARCH_PLAN_RELATIVE = "docs/phase12_research_plan.md"

FULL_SUITE_COMMAND = ".venv/bin/python -m unittest discover -s tests -q"
FOCUSED_SUITE_COMMAND = (
    ".venv/bin/python -m unittest "
    "tests.test_route_redundancy_ranking "
    "tests.test_service_route_redundancy "
    "tests.test_route_redundancy_ablations tests.test_ranking "
    "tests.test_service tests.test_profile_ranking "
    "tests.test_orchestration -q"
)
...
PHASE12_ORACLE_COMMAND = (
    ".venv/bin/python -m scripts.verify_phase12_route_redundancy_oracle"
)
...
ALLOWED_CANDIDATE_SOURCE_CHANGES = frozenset(
    {
        ...
        "scripts/verify_phase12_route_redundancy_oracle.py",
        "tests/test_route_redundancy_ablations.py",
        "tests/test_route_redundancy_ranking.py",
        "tests/test_service_route_redundancy.py",
    }
)
...
class SuiteConfig:
    ...
    def output(self) -> Path:
        return REPOSITORY_ROOT / f"results-phase12-{self.name}.json"

    @property
    def attempt(self) -> Path:
        return REPOSITORY_ROOT / f"results-phase12-{self.name}-attempt.json"
...
def _retained_agent_bytes(agent: ConversationalSearchAgent) -> int:
    ...
    candidate_counters = tuple(
        ...
        if name.startswith("_route_redundancy_")
    )
    retained = (
...
def _startup_probe(backend: object, *, iterations: int = 10_000) -> dict:
    ...
    for index in range(iterations):
        ...
        for policy in policies:
            ...
            if policy is STAGE_A_RANKING_POLICY:
                baseline = agent
    ...
    rss_candidate = ConversationalSearchAgent(
        "unused.jsonl",
        retriever=backend,
        ranking_policy=ROUTE_REDUNDANCY_CORRECTED_RANKING_POLICY,
    )
...
def _run_variant(
    ...
    _validate_phase12_accounting(diagnostics, ranking_policy)
...
def _warm_backend(catalog: Path, backend: object) -> None:
    ...
    session_id = "phase12-label-free-runtime-warmup"
...
def _validate_phase12_accounting(
    ...
    attempts = int(ranking["attempts"])  # type: ignore[index]
    if ranking["policy"] != ranking_policy.value:  # type: ignore[index]
        raise RuntimeError("ranking policy telemetry drifted")
    if int(rescue["attempts"]):  # type: ignore[index]
        raise RuntimeError("Phase 12 unexpectedly enabled Phase 10 rescue")
    outcomes = (
    ...
    redundancy_attempts = int(redundancy["attempts"])  # type: ignore[index]
    if sum(int(redundancy[key]) for key in outcomes) != redundancy_attempts:  # type: ignore[index]
        raise RuntimeError("route-redundancy outcomes do not partition attempts")
    if ranking_policy is ROUTE_REDUNDANCY_CORRECTED_RANKING_POLICY:
        if redundancy_attempts != attempts:
            raise RuntimeError("candidate route-redundancy coverage is incomplete")
    elif any(int(redundancy[key]) for key in ("attempts", *outcomes)):  # type: ignore[index]
    ...
    *,
    ...
    gates = {
        **_quality_gates(baseline, candidate, paired),
        ...
        "candidate_route_redundancy_outcomes_partition_attempts": (
            int(candidate.diagnostics["route_redundancy_health"]["attempts"])  # type: ignore[index]
            == int(candidate.diagnostics["ranking_health"]["attempts"])  # type: ignore[index]
        ),
        ...
        "candidate_additional_retained_agent_bytes_at_most_64kib": (
...
def _validate_prelock_verification(value: object) -> dict:
    expected_keys = {
        ...
        "phase12_oracle_command",
        "phase12_oracle_cases",
        "phase12_random_oracle_cases",
        "phase12_oracle_sha256",
        "completed_before_lock",
    }
    if not isinstance(value, dict) or set(value) != expected_keys:
        raise RuntimeError("Phase 12 pre-lock verification schema drifted")
    expected = {
        ...
        "phase12_oracle_command": PHASE12_ORACLE_COMMAND,
        "phase12_oracle_cases": 42_675,
        "phase12_random_oracle_cases": PHASE12_RANDOM_ORACLE_CASES,
        "phase12_oracle_sha256": PHASE12_ORACLE_SHA256,
        "completed_before_lock": True,
    }
    if any(value.get(key) != item for key, item in expected.items()):
        raise RuntimeError("Phase 12 pre-lock verification evidence drifted")
    for key in ("focused_tests_passed", "complete_unit_tests_passed"):
        if type(value.get(key)) is not int or value[key] <= 0:
            raise RuntimeError("Phase 12 pre-lock test count is invalid")
    return value
...
def _validate_implementation_lock(
    ...
    if not isinstance(lock, dict) or set(lock) != expected_keys:
        raise RuntimeError("Phase 12 implementation lock schema drifted")
    if lock.get("schema_version") != IMPLEMENTATION_LOCK_SCHEMA_VERSION:
        raise RuntimeError("unsupported Phase 12 implementation lock")
    if lock.get("lock_id") != IMPLEMENTATION_LOCK_ID:
        raise RuntimeError("unexpected Phase 12 implementation lock identity")
    if lock.get("status") != "locked_before_generator_evaluation":
        raise RuntimeError("Phase 12 candidate is not frozen")
    documents = {
    ...
    for key, relative in documents.items():
        ...
        if (
            ...
            raise RuntimeError("Phase 12 planning artifact drifted after lock")
    source_hashes = lock.get("source_sha256")
    if not isinstance(source_hashes, dict) or set(source_hashes) != set(SOURCE_PATHS):
        raise RuntimeError("Phase 12 source lock is incomplete")
    observed = {
        relative: _sha256(repository_root / relative)
        for relative in SOURCE_PATHS
    }
    if observed != source_hashes:
        raise RuntimeError("Phase 12 implementation drifted after lock")
...
def run_route_redundancy_ablation(
    suite: str,
    ...
    *,
    ...
    if suite not in SUITES:
        raise ValueError("unsupported Phase 12 suite")
    ...
    if not publication_privacy_is_valid(payload):
        raise RuntimeError("Phase 12 aggregate publication violates privacy")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run one frozen Phase 12 generator-separated suite"
    )
